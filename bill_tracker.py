...
# Search terms run against Open States full-text bill search per state.
# Add/remove terms here as EA's priorities shift — no code changes needed.
SEARCH_TERMS = [
...
def openstates_search(jurisdiction: str, query: str) -> list[dict]:
    ...
    try:
        ...
        print(f"  WARNING: Open States query failed ({jurisdiction} / '{query}'): {e}", file=sys.stderr)
        return []
...
def scan_state_bills(state_store: dict) -> tuple[list[dict], list[dict]]:
    ...
    for state_name in STATES:
        ...
        for term in SEARCH_TERMS:
            for b in openstates_search(state_name, term):
                if b["id"] not in seen_ids:
                    seen_ids.add(b["id"])
                    jurisdiction_results.append(b)
        ...
        for bill in jurisdiction_results:
            ...
            if prior is None:
                similar = find_similar_bills(bill, jurisdiction_results)
                summary = double_check_with_claude(record)
                new_filings.append({
                    **record,
                    "similar": similar,
